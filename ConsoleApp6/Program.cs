...
namespace NeuralNetwork
{
    ...
    class HiddenLayer : Layer
    {
        ...
        public override double[] BackwardPass(double[] gr_sums)
        {
            ...
            for (int i = 0; i < numofneurons; ++i)
                for (int n = 0; n < numofprevnerons; ++n)
    ...
    class OutputLayer : Layer
    {
        ...
        {
            double[] gr_sum = new double[numofprevnerons];
            for (int j = 0; j < gr_sum.Length; ++j)
            {
                sum += Neurons[KeyAvailable].Weights[j] * Neurons[k].Gradientor(errors[k], Neurons[k].Derivativator(Neurons[k].Output), 0);
                gr_sum[j] = sum;
            }
            for (int i = 0; i < numofneurons; ++i)
                for (int n = 0; n < numofprevnerons; ++n)
